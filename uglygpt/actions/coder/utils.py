...
class DocstringTransformer(cst.CSTTransformer):
    ...
    def _leave(self, original_node: DocstringNode, updated_node: DocstringNode) -> DocstringNode:
        ...
        if hasattr(updated_node, "decorators") and any((i.decorator.value == "overload") for i in updated_node.decorators):  # type: ignore
            return updated_node
...
class DocstringCollector(cst.CSTVisitor):
    ...
    def _leave(self, node: DocstringNode) -> None:
        key = tuple(self.stack)
        self.stack.pop()
        if hasattr(node, "decorators") and any(i.decorator.value == "overload" for i in node.decorators):  # type: ignore
            return
